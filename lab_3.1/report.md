...
|----------|---------|------------|
| 1 | `py grpc_server.py` | Запуск gRPC сервера |
| 2 | `py consumer.py` | Запуск Consumer (слушает очередь) |
| 3 | `py producer.py password:Qwerty123!` | Отправка задачи на проверку пароля |

---

## Вывод

В ходе выполнения лабораторной работы был реализован gRPC-сервис с тремя методами: проверка сложности пароля (`CheckPassword`), транслитерация русского текста в латиницу (`Transliterate`) и извлечение email-адресов из текста (`ExtractEmails`). Настроено асинхронное взаимодействие через брокер сообщений RabbitMQ: Producer отправляет задачи в очередь, Consumer забирает их и вызывает соответствующий метод gRPC-сервера. Все три метода успешно протестированы, RabbitMQ запущен в Docker-контейнере на локальной машине. Асинхронный подход позволяет развязать компоненты системы — задачи накапливаются в очереди и обрабатываются независимо от доступности отправителя.
